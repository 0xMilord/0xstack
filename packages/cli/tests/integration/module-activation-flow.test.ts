...
describe("Module Activation Tests - All 19 Modules Full Flow", () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "0xstack-modules-flow-"));
...
export async function getSeoConfig() {}`, "utf8");
  ...
  it("all 19 modules can be activated together without errors", async () => {
    // This is the ultimate integration test - activate everything at once
    await runModulesLifecycle(minimalCtx({
      seo: true,
      blogMdx: true,
      billing: "dodo",
    ...
    for (const file of filesToCheck) {
      const exists = await fs.access(path.join(tmpDir, file)).then(() => true).catch(() => false);
