...
import { runSync } from "../../src/core/sync/run-sync";

describe("Sync Command - Full Flow Tests", () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "0xstack-sync-flow-"));
...
export async function getSeoConfig() {}`, "utf8");
  ...
  it("sync plan detects missing deps", async () => {
    ...
    const result = await runSync({ projectRoot: tmpDir, profile: "core", apply: false });
    expect(result).toBeDefined();
    expect(result.missingDeps).toBeDefined();
    expect(result.missingDeps.length).toBeGreaterThan(0);
  }, 30_000);

  it("sync plan detects extra files from disabled modules", async () => {
    // Create a file that shouldn't exist when billing is disabled
    const billingFile = path.join(tmpDir, "app", "api", "v1", "billing", "checkout", "route.ts");
    await fs.mkdir(path.dirname(billingFile), { recursive: true });
    await fs.writeFile(billingFile, `export async function POST() {}`, "utf8");
  ...
  it("sync plan detects missing required files", async () => {
  ...
  it("sync plan returns clean result when everything is correct", async () => {
    const result = await runSync({ projectRoot: tmpDir, profile: "core", apply: false });
    expect(result).toBeDefined();
    expect(result.missingDeps.length).toBe(0);
    expect(result.extraFiles.length).toBe(0);
    expect(result.missingFiles.length).toBe(0);
  }, 30_000);

  it("sync plan detects multiple issues at once", async () => {
    // Remove a dep
    const pkgPath = path.join(tmpDir, "package.json");
    const pkg = JSON.parse(await fs.readFile(pkgPath, "utf8"));
    delete pkg.dependencies["better-auth"];
    ...
    // Add extra file from disabled module
    const billingFile = path.join(tmpDir, "app", "api", "v1", "billing", "checkout", "route.ts");
    await fs.mkdir(path.dirname(billingFile), { recursive: true });
    await fs.writeFile(billingFile, `export async function POST() {}`, "utf8");
  ...
  it("sync apply runs baseline when issues found", async () => {
    // Remove a dep to trigger baseline
    const pkgPath = path.join(tmpDir, "package.json");
    const pkg = JSON.parse(await fs.readFile(pkgPath, "utf8"));
    delete pkg.dependencies["better-auth"];
    await fs.writeFile(pkgPath, JSON.stringify(pkg, null, 2), "utf8");

    // Apply should run baseline
    await expect(runSync({ projectRoot: tmpDir, profile: "core", apply: true })).resolves.not.toThrow();
  }, 120_000);

  it("sync apply runs docs-sync when issues found", async () => {
    // Remove a dep to trigger baseline + docs-sync
    const pkgPath = path.join(tmpDir, "package.json");
    const pkg = JSON.parse(await fs.readFile(pkgPath, "utf8"));
    delete pkg.dependencies["better-auth"];
    await fs.writeFile(pkgPath, JSON.stringify(pkg, null, 2), "utf8");
