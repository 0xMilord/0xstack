...
11. Smoke test: temp directory `npm install 0xstack@X.Y.Z` and `npx 0xstack --help` against **https://registry.npmjs.org** (default **5s** warmup then retries for registry lag; avoids first-request ETARGET). Override with **`RELEASE_VERIFY_WARMUP_MS=0`** or skip entirely with **`RELEASE_SKIP_VERIFY=1`**
